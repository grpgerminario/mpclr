...
{synopt:{opt discretemiv}} specify that the MIV is a discrete or categorical variable; see below{p_end}
...
{phang}
{opt discretemiv} should be used when the MIV is discrete or categorical in nature. The {opt bins(#)} option
should also be used, where # is the number of unique values of the MIV. For example, if the MIV takes
on the values 1, 2, and 3, then {cmd:bins(3) discretemiv} should be included in the command options.
Note that the values should begin at 1 and be sequential.
By default, {cmd:mpclr} anticipates a continuous MIV.
