/*This is the 'mpbounds' subcommand, in which the non-MIV bounds are computed,
as well as the CLR-type bounding functions for the bound with MIV*/

program mpbounds, eclass properties(svyb)
	version 10.0
	#delimit ;
	syntax varlist (min=2 max=2 fv) [if] [in] [pw iw],  YMIN(string) YMAX(string) [ATT NMTS NMTR MIV(varname) bins(integer 5) DISCRETEMIV NMIV moret(numlist) mores(numlist)] UL(string) CONTRT(string) CONTRS(string) asmptn(string) ;
	marksample touse;
	#delimit cr
	...
	****************************************************************************
	...
	if "`att'" != "" {
		...
		forval t=`subminT'/`maxT' {
		    local k=`t'-1
			if "`asmptn'" == "wc" {
				qui matrix LB`t'`k'_wc = LB_`t'_`t'_wc - UB_`k'_`t'_wc
				qui matrix UB`t'`k'_wc = UB_`t'_`t'_wc - LB_`k'_`t'_wc
			}
			else if "`asmptn'" == "mts" {
				qui matrix LB`t'`k'_mts = LB_`t'_`t'_mts - UB_`k'_`t'_mts
				qui matrix UB`t'`k'_mts = UB_`t'_`t'_mts - LB_`k'_`t'_mts
			}
			else if "`asmptn'" == "mtr" {
				qui matrix LB`t'`k'_mtr = LB_`t'_`t'_mtr - UB_`k'_`t'_mtr
				qui matrix UB`t'`k'_mtr = UB_`t'_`t'_mtr - LB_`k'_`t'_mtr
			}
			else if "`asmptn'" == "mtsr" {
				qui matrix LB`t'`k'_mtsr = LB_`t'_`t'_mtsr - UB_`k'_`t'_mtsr
				qui matrix UB`t'`k'_mtsr = UB_`t'_`t'_mtsr - LB_`k'_`t'_mtsr
			}
			
		}
		
		/*Collect info on 'more' ATTs, if requested*/
		...
		if "`moret'" != "" {
			...
			if ("`nmts'" != "" & "`nmtr'" == "") {
				forval i=1/`conta' {
					if "`asmptn'" == "wc" {
						qui scalar LB_`s`i''_`t`i''_wc = `ymin'
						qui scalar UB_`s`i''_`t`i''_wc = `ymax'
						qui scalar LB_`t`i''_`t`i''_wc = ${m_y`t`i''}
						qui scalar UB_`t`i''_`t`i''_wc = ${m_y`t`i''}

					}
					else if "`asmptn'" == "mts" {
						qui scalar LB_`s`i''_`t`i''_mts = `ymin'
						qui scalar UB_`s`i''_`t`i''_mts = ${m_y`s`i''}
						qui scalar LB_`t`i''_`t`i''_mts = ${m_y`t`i''}
						qui scalar UB_`t`i''_`t`i''_mts = ${m_y`t`i''}
					}
					else if "`asmptn'" == "mtr" {
						qui scalar LB_`s`i''_`t`i''_mtr = `ymin'
						qui scalar UB_`s`i''_`t`i''_mtr = ${m_y`t`i''}
						qui scalar LB_`t`i''_`t`i''_mtr = ${m_y`t`i''}
						qui scalar UB_`t`i''_`t`i''_mtr = ${m_y`t`i''}
					}
					else if "`asmptn'" == "mtsr" {
						qui scalar LB_`s`i''_`t`i''_mtsr = `ymin'
						qui scalar UB_`s`i''_`t`i''_mtsr = min(${m_y`t`i''},${m_y`s`i''})
						qui scalar LB_`t`i''_`t`i''_mtsr = ${m_y`t`i''}
						qui scalar UB_`t`i''_`t`i''_mtsr = ${m_y`t`i''}
					}
				}
			}
			...
			/*Non-negative MTS + non-positive MTR*/
				...
				forval i=1/`conta' {
					if "`asmptn'" == "wc" {
						...
						qui scalar LB_`s`i''_`t`i''_mts = ${m_y`s`i''}
						qui scalar UB_`s`i''_`t`i''_mts = `ymax'
						qui scalar LB_`t`i''_`t`i''_mts = ${m_y`t`i''}
						qui scalar UB_`t`i''_`t`i''_mts = ${m_y`t`i''}
					}
					else if "`asmptn'" == "mtr" {
						qui scalar LB_`s`i''_`t`i''_mtr = ${m_y`t`i''}
						qui scalar UB_`s`i''_`t`i''_mtr = `ymax'
						qui scalar LB_`t`i''_`t`i''_mtr = ${m_y`t`i''}
						qui scalar UB_`t`i''_`t`i''_mtr = ${m_y`t`i''}
					}
					else if "`asmptn'" == "mtsr" {
						qui scalar LB_`s`i''_`t`i''_mtsr = max(${m_y`t`i''},${m_y`s`i''})
						qui scalar UB_`s`i''_`t`i''_mtsr = `ymax'
						qui scalar LB_`t`i''_`t`i''_mtsr = ${m_y`t`i''}
						qui scalar UB_`t`i''_`t`i''_mtsr = ${m_y`t`i''}
					}
				}
			...
			/*Non-positive MTS + non-positive MTR*/
				...
				forval i=1/`conta' {		
					if "`asmptn'" == "wc" {
						...
						qui scalar LB_`s`i''_`t`i''_mtsr = ${m_y`t`i''}
						qui scalar UB_`s`i''_`t`i''_mtsr = ${m_y`s`i''}
						qui scalar LB_`t`i''_`t`i''_mtsr = ${m_y`t`i''}
						qui scalar UB_`t`i''_`t`i''_mtsr = ${m_y`t`i''}
					}
				}
			...
			/*Non-negative MTS + non-negative MTR*/
				...
				forval i=1/`conta' {	
					if "`asmptn'" == "wc" {
						...
						qui scalar LB_`s`i''_`t`i''_mtsr = ${m_y`s`i''}
						qui scalar UB_`s`i''_`t`i''_mtsr = ${m_y`t`i''}
						qui scalar LB_`t`i''_`t`i''_mtsr = ${m_y`t`i''}
						qui scalar UB_`t`i''_`t`i''_mtsr = ${m_y`t`i''}
					}
				}
			...
			/*Estimate the corresponding 'more' bounds*/
			
			forval i=1/`conta' {
				if "`asmptn'" == "wc" {
					qui matrix LB`t`i''`s`i''_wc = LB_`t`i''_`t`i''_wc - UB_`s`i''_`t`i''_wc
					qui matrix UB`t`i''`s`i''_wc = UB_`t`i''_`t`i''_wc - LB_`s`i''_`t`i''_wc
				}
				else if "`asmptn'" == "mts" {
					qui matrix LB`t`i''`s`i''_mts = LB_`t`i''_`t`i''_mts - UB_`s`i''_`t`i''_mts
					qui matrix UB`t`i''`s`i''_mts = UB_`t`i''_`t`i''_mts - LB_`s`i''_`t`i''_mts
				}
				else if "`asmptn'" == "mtr" {
					qui matrix LB`t`i''`s`i''_mtr = LB_`t`i''_`t`i''_mtr - UB_`s`i''_`t`i''_mtr
					qui matrix UB`t`i''`s`i''_mtr = UB_`t`i''_`t`i''_mtr - LB_`s`i''_`t`i''_mtr
				}
				else if "`asmptn'" == "mtsr" {
					qui matrix LB`t`i''`s`i''_mtsr = LB_`t`i''_`t`i''_mtsr - UB_`s`i''_`t`i''_mtsr
					qui matrix UB`t`i''`s`i''_mtsr = UB_`t`i''_`t`i''_mtsr - LB_`s`i''_`t`i''_mtsr
				}
			}
		} 
	restore 
	...
	/*First, estimate all bounds on mean potential outcomes, not involving the MIV*/
		...
		/*Bounds on the ATE, no MIV*/
		
		forval t=`subminT'/`maxT' {
		    local k=`t'-1
			if "`asmptn'" == "wc" {
				qui matrix LB`t'`k'_wc = LB_`t'_wc - UB_`k'_wc
				qui matrix UB`t'`k'_wc = UB_`t'_wc - LB_`k'_wc
			}
			else if "`asmptn'" == "mts" {
				qui matrix LB`t'`k'_mts = LB_`t'_mts - UB_`k'_mts
				qui matrix UB`t'`k'_mts = UB_`t'_mts - LB_`k'_mts
			}
			else if "`asmptn'" == "mtr" {
				qui matrix LB`t'`k'_mtr = LB_`t'_mtr - UB_`k'_mtr
				qui matrix UB`t'`k'_mtr = UB_`t'_mtr - LB_`k'_mtr
			}
			else if "`asmptn'" == "mtsr" {
				qui matrix LB`t'`k'_mtsr = LB_`t'_mtsr - UB_`k'_mtsr
				qui matrix UB`t'`k'_mtsr = UB_`t'_mtsr - LB_`k'_mtsr
			}
			
		}
		
		/*Collect info on 'more' ATEs, if requested*/
		...
		if "`moret'" != "" {
			...
			forval i=1/`conta' {
				if "`asmptn'" == "wc" {
					qui matrix LB`t`i''`s`i''_wc = LB_`t`i''_wc - UB_`s`i''_wc
					qui matrix UB`t`i''`s`i''_wc = UB_`t`i''_wc - LB_`s`i''_wc
				}
				else if "`asmptn'" == "mts" {
					qui matrix LB`t`i''`s`i''_mts = LB_`t`i''_mts - UB_`s`i''_mts
					qui matrix UB`t`i''`s`i''_mts = UB_`t`i''_mts - LB_`s`i''_mts
				}
				else if "`asmptn'" == "mtr" {
					qui matrix LB`t`i''`s`i''_mtr = LB_`t`i''_mtr - UB_`s`i''_mtr
					qui matrix UB`t`i''`s`i''_mtr = UB_`t`i''_mtr - LB_`s`i''_mtr
				}
				else if "`asmptn'" == "mtsr" {
					qui matrix LB`t`i''`s`i''_mtsr = LB_`t`i''_mtsr - UB_`s`i''_mtsr
					qui matrix UB`t`i''`s`i''_mtsr = UB_`t`i''_mtsr - LB_`s`i''_mtsr
				}
			}
		} 
	restore 
	...
	if "`miv'" != "" {
		...
		if "`moret'" != "" {
					...
					if ("`nmts'" != "" & "`nmtr'" == "") {
						forval i=1/`conta' {
							if "`asmptn'" == "wcv" {
								qui scalar LB_`s`i''_`t`i''_wcv_`j' = `ymin'
								qui scalar UB_`s`i''_`t`i''_wcv_`j' = `ymax'
								qui scalar LB_`t`i''_`t`i''_wcv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`t`i''_`t`i''_wcv_`j' = ${m_y`t`i''_`j'}

							}
							else if "`asmptn'" == "mtsv" {
								qui scalar LB_`s`i''_`t`i''_mtsv_`j' = `ymin'
								qui scalar UB_`s`i''_`t`i''_mtsv_`j' = ${m_y`s`i''_`j'}
								qui scalar LB_`t`i''_`t`i''_mtsv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`t`i''_`t`i''_mtsv_`j' = ${m_y`t`i''_`j'}
							}
							else if "`asmptn'" == "mtrv" {
								qui scalar LB_`s`i''_`t`i''_mtrv_`j' = `ymin'
								qui scalar UB_`s`i''_`t`i''_mtrv_`j' = ${m_y`t`i''_`j'}
								qui scalar LB_`t`i''_`t`i''_mtrv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`t`i''_`t`i''_mtrv_`j' = ${m_y`t`i''_`j'}
							}
							else if "`asmptn'" == "mtsrv" {
								qui scalar LB_`s`i''_`t`i''_mtsrv_`j' = `ymin'
								qui scalar UB_`s`i''_`t`i''_mtsrv_`j' = min(${m_y`t`i''_`j'},${m_y`s`i''_`j'})
								qui scalar LB_`t`i''_`t`i''_mtsrv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`t`i''_`t`i''_mtsrv_`j' = ${m_y`t`i''_`j'}
							}
						}
					}
					...
					/*Non-negative MTS + non-positive MTR*/
						...
						forval i=1/`conta' {
							if "`asmptn'" == "wcv" {
								...
								qui scalar LB_`s`i''_`t`i''_mtsv_`j' = ${m_y`s`i''_`j'}
								qui scalar UB_`s`i''_`t`i''_mtsv_`j' = `ymax'
								qui scalar LB_`t`i''_`t`i''_mtsv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`t`i''_`t`i''_mtsv_`j' = ${m_y`t`i''_`j'}
							}
							else if "`asmptn'" == "mtrv" {
								qui scalar LB_`s`i''_`t`i''_mtrv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`s`i''_`t`i''_mtrv_`j' = `ymax'
								qui scalar LB_`t`i''_`t`i''_mtrv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`t`i''_`t`i''_mtrv_`j' = ${m_y`t`i''_`j'}
							}
							else if "`asmptn'" == "mtsrv" {
								qui scalar LB_`s`i''_`t`i''_mtsrv_`j' = max(${m_y`t`i''_`j'},${m_y`s`i''_`j'})
								qui scalar UB_`s`i''_`t`i''_mtsrv_`j' = `ymax'
								qui scalar LB_`t`i''_`t`i''_mtsrv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`t`i''_`t`i''_mtsrv_`j' = ${m_y`t`i''_`j'}
							}
						}
					...
					/*Non-positive MTS + non-positive MTR*/
						...
						forval i=1/`conta' {		
							if "`asmptn'" == "wcv" {
								...
								qui scalar LB_`s`i''_`t`i''_mtsrv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`s`i''_`t`i''_mtsrv_`j' = ${m_y`s`i''_`j'}
								qui scalar LB_`t`i''_`t`i''_mtsrv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`t`i''_`t`i''_mtsrv_`j' = ${m_y`t`i''_`j'}
							}
						}
					...
					/*Non-negative MTS + non-negative MTR*/
						...
						forval i=1/`conta' {	
							if "`asmptn'" == "wcv" {
								...
								qui scalar LB_`s`i''_`t`i''_mtsrv_`j' = ${m_y`s`i''_`j'}
								qui scalar UB_`s`i''_`t`i''_mtsrv_`j' = ${m_y`t`i''_`j'}
								qui scalar LB_`t`i''_`t`i''_mtsrv_`j' = ${m_y`t`i''_`j'}
								qui scalar UB_`t`i''_`t`i''_mtsrv_`j' = ${m_y`t`i''_`j'}
							}
						}
	...
	if "`nmiv'" != "" {
		...
		if "`ul'" == "U" {
			preserve
				...
				qui gen id=_n
				qui gen pos0 = UB_`contrt'_`contrt'_`asmptn'_1
				global sorterU pos0
				...
				forval j=1/`bins' {
					...
					qui replace pos`j' = UB_`contrt'_`contrt'_`asmptn'_`j' if half`j'==2
					
					qui drop half`j'
					global sorterU ${sorterU} pos`j'
				}
						
				mkmat pos1-pos`bins', mat(U_`contrt'_`contrt'_`asmptn')
			restore
		...
		/*Prepare lower bound bounding functions*/
			...
			preserve
				...
				qui gen id=_n
				local init = `bins' + 1
				qui gen pos`init' = -LB_`contrt'_`contrt'_`asmptn'_`bins'
				global sorterL pos`init'
				...
				forval j=`bins' (-1) 1 {
					...
					qui egen half`j' = xtile(id), by(pos`i') nq(2)
					qui gen pos`j' = pos`i' if half`j'==1
					qui replace pos`j' = -LB_`contrt'_`contrt'_`asmptn'_`j' if half`j'==2
							
					qui drop half`j'
					global sorterL ${sorterL} pos`j'
				}
						
				mkmat pos`bins'-pos1, mat(L_`contrt'_`contrt'_`asmptn')
			restore
				
			matselrc L_`contrt'_`contrt'_`asmptn' L_`contrt'_`contrt'_`asmptn', c(`bins'/1)
			matrix L_`contrt'_`contrt'_`asmptn' = -L_`contrt'_`contrt'_`asmptn'
		}	
		}
		...
		if "`ul'" == "U" {
			...
			preserve
				...
				qui gen id=_n
				local init = `bins' + 1
				qui gen pos`init' = -LB_`contrs'_`asmptn'_`bins'
				global sorterL pos`init'
				...
				forval j=`bins' (-1) 1 {
					...
					qui egen half`j' = xtile(id), by(pos`i') nq(2)
					qui gen pos`j' = pos`i' if half`j'==1
					qui replace pos`j' = -LB_`contrs'_`asmptn'_`j' if half`j'==2
							
					qui drop half`j'
					global sorterL ${sorterL} pos`j'
				}
						
				mkmat pos`bins'-pos1, mat(L_`contrs'_`asmptn')
			restore
				
			matselrc L_`contrs'_`asmptn' L_`contrs'_`asmptn', c(`bins'/1)
			matrix L_`contrs'_`asmptn' = -L_`contrs'_`asmptn'
		}
		...
		/*Prepare lower bound bounding functions*/
			...
			preserve
				...
				qui gen id=_n
				qui gen pos0 = UB_`contrs'_`asmptn'_1
				global sorterU pos0
				...
				forval j=1/`bins' {
					...
					qui egen half`j' = xtile(id), by(pos`i') nq(2)
					qui gen pos`j' = pos`i' if half`j'==1
					qui replace pos`j' = UB_`contrs'_`asmptn'_`j' if half`j'==2
					
					qui drop half`j'
					global sorterU ${sorterU} pos`j'
				}
						
				mkmat pos1-pos`bins', mat(U_`contrs'_`asmptn')
			restore
	...
	/*Non-negative MIV*/
		...
		if "`ul'" == "L" {
			preserve
				...
				qui gen id=_n
				qui gen pos0 = LB_`contrt'_`contrt'_`asmptn'_1
				global sorterL pos0
				...
				mkmat pos1-pos`bins', mat(L_`contrt'_`contrt'_`asmptn')
			restore
		...
		/*Prepare upper bound bounding functions*/
			...
			preserve
				...
				qui gen id=_n
				local init = `bins' + 1
				qui gen pos`init' = -UB_`contrt'_`contrt'_`asmptn'_`bins'
				global sorterU pos`init'
				...
				mkmat pos`bins'-pos1, mat(U_`contrt'_`contrt'_`asmptn')
			restore
				
			matselrc U_`contrt'_`contrt'_`asmptn' U_`contrt'_`contrt'_`asmptn', c(`bins'/1)
			matrix U_`contrt'_`contrt'_`asmptn' = -U_`contrt'_`contrt'_`asmptn'
		}			
		}
		...
		if "`ul'" == "L" {
			...
			preserve
				...
				qui gen id=_n
				local init = `bins' + 1
				qui gen pos`init' = -UB_`contrs'_`asmptn'_`bins'
				global sorterU pos`init'
				...
				mkmat pos`bins'-pos1, mat(U_`contrs'_`asmptn')
			restore
				
			matselrc U_`contrs'_`asmptn' U_`contrs'_`asmptn', c(`bins'/1)
			matrix U_`contrs'_`asmptn' = -U_`contrs'_`asmptn'
		}
		...
		/*Prepare upper bound bounding functions*/
			...
			preserve
				...
				qui gen id=_n
				qui gen pos0 = LB_`contrs'_`asmptn'_1
				global sorterL pos0
				...
				mkmat pos1-pos`bins', mat(L_`contrs'_`asmptn')
			restore
	...
	if "`att'" != "" {
		if "`ul'" == "L" {
			matrix LL_`contrt'_`contrt'_`asmptn' = L_`contrt'_`contrt'_`asmptn'*Pj
			matrix UU_`contrs'_`contrt'_`asmptn' = U_`contrs'_`contrt'_`asmptn'*Pj
		}
		if "`ul'" == "U" {
			matrix UU_`contrt'_`contrt'_`asmptn' = U_`contrt'_`contrt'_`asmptn'*Pj
		...
		if "`ul'" == "L" {
			...
			matrix UU_`contrs'_`asmptn' = U_`contrs'_`asmptn'*Pj
		}
		if "`ul'" == "U" {
			matrix UU_`contrt'_`asmptn' = U_`contrt'_`asmptn'*Pj
			matrix LL_`contrs'_`asmptn' = L_`contrs'_`asmptn'*Pj
		}
	}
	...
	if "`att'" != "" {
		if "`ul'" == "L" {
			mata: LL_`contrt'_`contrt'_`asmptn' = st_matrix("LL_`contrt'_`contrt'_`asmptn'")

			mata: UU_`contrs'_`contrt'_`asmptn' = st_matrix("UU_`contrs'_`contrt'_`asmptn'")
					
			mata: UUs_`contrs'_`contrt'_`asmptn' = UU_`contrs'_`contrt'_`asmptn'
			mata: for (i=2; i<=tterms; i++) UUs_`contrs'_`contrt'_`asmptn' = (UUs_`contrs'_`contrt'_`asmptn' \ UU_`contrs'_`contrt'_`asmptn')
		
			mata: LB`contrt'`contrs'_`asmptn' = (LL_`contrt'_`contrt'_`asmptn'#J(tterms,1,1) - UUs_`contrs'_`contrt'_`asmptn')'
			
			mata: LB`contrt'`contrs'_`asmptn' = distinctrowsof((LB`contrt'`contrs'_`asmptn')')'
		
			mata: LB`contrt'`contrs'_`asmptn' = (LB`contrt'`contrs'_`asmptn', max(LB`contrt'`contrs'_`asmptn'))
		
			mata: st_matrix("LB`contrt'`contrs'_`asmptn'",LB`contrt'`contrs'_`asmptn')
		}
	
		if "`ul'" == "U" {
			mata: UU_`contrt'_`contrt'_`asmptn' = st_matrix("UU_`contrt'_`contrt'_`asmptn'")
		
			mata: LL_`contrs'_`contrt'_`asmptn' = st_matrix("LL_`contrs'_`contrt'_`asmptn'")
		
			mata: LLs_`contrs'_`contrt'_`asmptn' = LL_`contrs'_`contrt'_`asmptn'
			mata: for (i=2; i<=tterms; i++) LLs_`contrs'_`contrt'_`asmptn' = (LLs_`contrs'_`contrt'_`asmptn' \ LL_`contrs'_`contrt'_`asmptn')
		
			mata: UB`contrt'`contrs'_`asmptn' = (UU_`contrt'_`contrt'_`asmptn'#J(tterms,1,1) - LLs_`contrs'_`contrt'_`asmptn')'
			
			mata: UB`contrt'`contrs'_`asmptn' = distinctrowsof((UB`contrt'`contrs'_`asmptn')')'
		
			mata: UB`contrt'`contrs'_`asmptn' = (UB`contrt'`contrs'_`asmptn', min(UB`contrt'`contrs'_`asmptn'))
			
			mata: st_matrix("UB`contrt'`contrs'_`asmptn'",UB`contrt'`contrs'_`asmptn')
		}
	}
		...
		if "`ul'" == "L" {
			...
			mata: UU_`contrs'_`asmptn' = st_matrix("UU_`contrs'_`asmptn'")
					
			mata: UUs_`contrs'_`asmptn' = UU_`contrs'_`asmptn'
			mata: for (i=2; i<=tterms; i++) UUs_`contrs'_`asmptn' = (UUs_`contrs'_`asmptn' \ UU_`contrs'_`asmptn')
		
			mata: LB`contrt'`contrs'_`asmptn' = (LL_`contrt'_`asmptn'#J(tterms,1,1) - UUs_`contrs'_`asmptn')'
		...
		if "`ul'" == "U" {
			...
			mata: LL_`contrs'_`asmptn' = st_matrix("LL_`contrs'_`asmptn'")
		
			mata: LLs_`contrs'_`asmptn' = LL_`contrs'_`asmptn'
			mata: for (i=2; i<=tterms; i++) LLs_`contrs'_`asmptn' = (LLs_`contrs'_`asmptn' \ LL_`contrs'_`asmptn')
		
			mata: UB`contrt'`contrs'_`asmptn' = (UU_`contrt'_`asmptn'#J(tterms,1,1) - LLs_`contrs'_`asmptn')'
	...
	if "`ul'" == "L" {	
		ereturn post LB`contrt'`contrs'_`asmptn', esample(`touse') properties("b")
	}
	
	if "`ul'" == "U" {
		ereturn post UB`contrt'`contrs'_`asmptn', esample(`touse') properties("b")
	}
