...
program mpbounds, eclass properties(svyb)
	version 10.0
	#delimit ;
	syntax varlist (min=2 max=2 fv) [if] [in] [pw iw],  YMIN(string) YMAX(string) [ATT NMTS NMTR MIV(varname) bins(integer 5) NMIV moret(numlist) mores(numlist)] UL(string) CONTRT(string) CONTRS(string) asmptn(string) ;
	marksample touse;
	#delimit cr
