program mpclr, eclass properties(svyb)
	version 10.0
	#delimit ;
	syntax varlist (min=2 max=2 fv) [if] [in] [pw iw],  YMIN(string) YMAX(string) [ATT BOUNDS(string) NMTS NMTR MIV(varname) MIVBOUNDS(string) bins(integer 5) DISCRETEMIV NMIV moret(numlist) mores(numlist) reps(integer 25) seed(integer 22176) level(integer 95) NOISYCLR LATEX(string) NPSU(int -1) SURVEY BRR UNCORRECTED];
	marksample touse;
	#delimit cr
			...
			foreach a of local asmpt {
				forval t=`subminT'/`maxT' {
					foreach x of local lowup {
						...
						bootstrap _b, reps(`reps') seed(`seed') saving(`cosav_`x'_`t'_`a''.dta, double replace) noheader notable:  mpbounds `y' `T' if `touse', ymin(`ymin') ymax(`ymax') `att' `nmts' `nmtr' miv(`miv') bins(`bins') `discretemiv' `nmiv' moret(`moret') mores(`mores') ul(`x') contrt(`t') contrs(`k') asmptn(`a')
				...
				if "`moret'" != "" {
					forval i=1/`conta' {
						foreach x of local lowup {
							...
							bootstrap _b, reps(`reps') seed(`seed') saving(`cosavm_`x'_`i'_`a''.dta, double replace) noheader notable:  mpbounds `y' `T' if `touse', ymin(`ymin') ymax(`ymax') `att' `nmts' `nmtr' miv(`miv') bins(`bins') `discretemiv' `nmiv' moret(`moret') mores(`mores') ul(`x') contrt(`t`i'') contrs(`s`i'') asmptn(`a')
